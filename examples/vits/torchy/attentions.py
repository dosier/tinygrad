...
from examples.vits.torchy import commons
from examples.vits.torchy.modules import LayerNorm
...
class MultiHeadAttention(nn.Module):
  ...
  def attention(self, query, key, value, mask=None):
    ...
    if self.window_size is not None:
      ...
      debug(key_relative_embeddings, "MultiHeadAttention.attention[0](key_relative_embeddings)")
      rel_logits = self._matmul_with_relative_keys(query /math.sqrt(self.k_channels), key_relative_embeddings)
      debug(rel_logits, "MultiHeadAttention.attention[0](rel_logits)")
      scores_local = self._relative_position_to_absolute_position(rel_logits)
      debug(scores_local, "MultiHeadAttention.attention[0](scores_local)")
