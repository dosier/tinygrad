...
from examples.vits.torchy import attentions, modules, commons
from examples.vits.torchy.commons import init_weights, get_padding
...
class TextEncoder(nn.Module):
  ...
  def forward(self, x, x_lengths):
    debug(x, "TextEncoder.forward[0](x)")
    x = self.emb(x)
    debug(x, "TextEncoder.forward[1](x)")
    x = x * math.sqrt(self.hidden_channels) # [b, t, h]
    x = torch.transpose(x, 1, -1) # [b, h, t]
    debug(x, "TextEncoder.forward[2](x)")
    x_mask = torch.unsqueeze(commons.sequence_mask(x_lengths, x.size(2)), 1).to(x.dtype)
    debug(x_mask, "TextEncoder.forward[3](x_mask)")
    x = self.encoder(x * x_mask, x_mask)
    debug(x, "TextEncoder.forward[4](x)")
    stats = self.proj(x) * x_mask
    debug(stats, "TextEncoder.forward[5](stats)")
    m, logs = torch.split(stats, self.out_channels, dim=1)
    debug(m, "TextEncoder.forward[6](m)")
...
class ResidualCouplingBlock(nn.Module):
  ...
  def forward(self, x, x_mask, g=None, reverse=False):
    # print(f"ResidualCouplingBlock.forward[0](x={x.shape}, x_mask={x_mask.shape}, g={g.shape if g is not None else None}, reverse={reverse})")
    i = 0
    if not reverse:
      for flow in self.flows:
        x, _ = flow.forward(x, x_mask, g=g, reverse=reverse)
        debug(x, f"ResidualCouplingBlock.forward[1][{i}](x)")
        i += 1
    else:
